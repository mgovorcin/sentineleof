...
Accepts a .SAFE directory or a zipped SAFE (any extension).
...
def get_relative_orbit(safe_path: str) -> dict:
    ...
    # Parse mission from filename via the same regex used elsewhere in eof
    satellite = Sentinel(Path(safe_path).stem).mission
...
if __name__ == "__main__":
    ...
    if "derived_offset_mod175" in info:
        ...
        print(
            f"Formula check:        ({info['absolute_orbit']} - {info['derived_offset_mod175']}) % 175 + 1 = {info['formula_check']}"
        )
